...
const CATEGORIAS = ['Aluguel', 'Mercadorias', 'Utilidades', 'Serviços', 'RH', 'Marketing', 'Impostos', 'Outros']
const STATUS_OPTS = ['pendente', 'pago', 'vencido']

const statusConfig = {
    pago: { label: 'Pago', className: 'badge-success', icon: CheckCircle, color: 'var(--success)' },
    pendente: { label: 'Pendente', className: 'badge-warning', icon: Clock, color: 'var(--warning)' },
    vencido: { label: 'Vencido', className: 'badge-danger', icon: AlertCircle, color: 'var(--danger)' },
}
            ...
            <div className="modal-sheet" onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div>
                        ...
                        <input className="input-field" placeholder="Ex: Aluguel Loja" value={form.descricao} onChange={e => set('descricao', e.target.value)} />
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                        <div>
                            <label style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '500', display: 'block', marginBottom: '6px' }}>Categoria</label>
...
export default function ContasPagar() {
    const { data, addContaPagar, updateContaPagar, deleteContaPagar } = useApp()
    ...
    const totalPendente = filtered.filter(c => c.status !== 'pago').reduce((a, c) => a + c.valor, 0)

    const openEdit = (item) => { setEditItem(item); setModal(true) }

    return (
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Header title="Contas a Pagar" subtitle={`${filtered.length} conta${filtered.length !== 1 ? 's' : ''}`} />
            ...
            <div className="page-content">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
                    ...
                    <div className="card" style={{ padding: '12px', background: 'rgba(var(--danger-rgb), 0.08)', borderColor: 'rgba(var(--danger-rgb), 0.2)' }}>
                        <p style={{ fontSize: '11px', color: 'var(--text-muted)', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Pendente</p>
                        <p style={{ fontSize: '18px', fontWeight: '700', color: 'var(--danger)', marginTop: '2px' }}>{fmt(totalPendente)}</p>
                    </div>
                </div>
                ...
                <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
                    {filtered.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-muted)' }}>
                            ...
                            <p style={{ fontSize: '14px' }}>Nenhuma conta encontrada</p>
                        </div>
                            ...
                            >
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>
                                        {conta.fornecedor && `${conta.fornecedor} • `}{conta.categoria} • {new Date(conta.vencimento + 'T00:00:00').toLocaleDateString('pt-BR')}
                                    </p>
                                </div>
                                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                    <p style={{ fontSize: '14px', fontWeight: '700', color: conta.status === 'pago' ? 'var(--text-muted)' : 'var(--danger)' }}>
                                        {fmt(conta.valor)}
                                    </p>
            ...
            {modal && (
                <FormModal
                    ...
                    onSave={editItem ? (d) => updateContaPagar(editItem.id, d) : addContaPagar}
